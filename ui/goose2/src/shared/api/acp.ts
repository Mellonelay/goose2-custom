...
export async function acpPrepareSession(
  ...
  const result = await sessionTracker.prepareSession(
    sessionId,
    providerId,
    workingDir,
    options.personaId,
  ...
  if (result.configOptions && result.configOptions.length > 0) {
    applySessionConfigOptions(sessionId, result.configOptions, providerId);
  }
  perfLog(
...
export async function acpSetModel(
  ...
  const response = await directAcp.setModel(
    gooseSessionId ?? sessionId,
    modelId,
  );
  if (response.configOptions.length > 0) {
    applySessionConfigOptions(sessionId, response.configOptions);
  }
}
