# Local DaVinci Override for This Workspace

Before acting in this repository, read these in order:

1. `DAVINCI_CONSTITUTION.md`
2. `ui/goose2/AGENTS.md`
3. this file

Local priority for this workspace:
- stabilize Goose2 on Windows ARM64
- preserve the validated `goose.exe` -> `goose serve` -> ACP path
- preserve ACP-first truth
- prefer smallest-real-diff fixes
- validate with real runtime/UI behavior when possible

Local override rules:
- If working in `ui/goose2`, follow `ui/goose2/AGENTS.md` over legacy `ui/desktop` guidance.
- Do not reintroduce `goosed.exe`, Electron-era desktop assumptions, or REST/SSE desktop paths for Goose2.
- Do not patch UI symptoms first when runtime/session truth is broken.
- For provider/model/session bugs, trace:
  runtime/ACP -> API/handler boundary -> store -> selector -> UI
- Keep fixes surgical and architecture-preserving.

This override supplements the repository instructions below and does not replace them.
